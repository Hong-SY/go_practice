			...
			if n.Type == html.ElementNode {
				for _, a := range n.Attr {
					...
					if a.Key == "class" && a.Val == "Link--secondary pl-1" {
						...
						g.Request("https://github.com/" + user + "?tab=following")
						g.stars.Request("https://github.com/" + user + "?tab=stars")
						break
					}
				}
			}
...
func (g *GitHubStars) Parse(doc *html.Node) <-chan string {
	...
	go func() {
		...
		f = func(n *html.Node) {
			if n.Type == html.ElementNode {
				for _, a := range n.Attr {
					if a.Key == "class" && a.Val == "d-inline-block mb-1" {
						dat := n.FirstChild.NextSibling.FirstChild.NextSibling.Attr[0].Val
						repo <- dat
						break
					}
				}
...
func main() {
	...
	following := &GitHubFollowing{
		...
		stars:      stars,
		url:        "https://github.com/huonghope?tab=following",
	}
	p.request <- following
	following.stars.Request("https://github.com/huonghope?tab=stars")
